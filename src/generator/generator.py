...
class Generator(ABC):
    ...
    def __init__(
        ...
        dataAugmenter = DataAugmenter(augmentations=augmentation_config)
        self.transforms = (
            [(None, None)] + list(dataAugmenter.get_operations().items())
            if augmentation
            else [(None, None)]
        )
    ...
    def generate_data(self, tasks: dict) -> None:
        """
        ...
        with yaspin(text=f"Creating dataloader", spinner=Spinners.line) as spinner:
            dataloader = Dataloader(datasets=self.datasets, dict=self.dict)
        ...
        print(
            f"{Fore.LIGHTCYAN_EX}[Training data generation for {self.name()}]{Fore.RESET}"
        )
        for task_name in tasks.keys():
            if tasks[task_name] == "y":
                with yaspin(
                    text=f"Generating {task_name} data", spinner=Spinners.line
                ) as spinner:
                    ...
                    spinner.text = (
                        f"Generated {task_name} data in {end_time - start_time:.2f} s"
                    )
                    spinner.ok(f"{Fore.GREEN}✓{Fore.RESET}")

    def run_process(
        self, img_output_path: str, dataloader: Dataloader, task: str, split: str
    ) -> list:
        process_map = {"Detection": self._det, "Recognition": self._rec}
        results = []
        for transform in self.transforms:
            args = [
                (img_output_path, img_path, gt, transform)
                for (img_path, gt) in dataloader.data[split]
            ]
